    ...
    h(
      "button",
      {
        ...
        style: {
          ...
          background: node.type === "profile" ? C.accent_profile: "transparent",
          color: node.type === "profile" ? C.text_light : C.text, fontWeight: 600,
        },
      },
      IconUser(13), " Profile"
    ),
    h(
      "button",
      {
        ...
        style: {
          display: "flex", alignItems: "center", gap: 5, padding: "7px 10px",
          border: "none", cursor: "pointer", fontSize: 12,
          background: node.type === "group" ? C.accent_group : "transparent",
          color: node.type === "group" ? C.text_light : C.text, fontWeight: 600,
        },
      },
      IconLayers(13), " Group"
    )
...
function BuilderPanel() {
  ...
  return h(
    ...
    h(
      ...
      {
        style: {
          //this is the style for the builder pannel with logic. The UI felt to busy, so this has been set to 0px, althought it technically exists
          background: C.panel, border: `0px solid ${C.border}`,
      ...
      h(
        ...
        h(
          "button",
          {
            ...
            style: {
              display: "flex", alignItems: "center", gap: 6,
              background: ok ? C.accent : C.panel2, color: ok ? C.text_light : C.dim,
              border: `1px solid ${C.border}`, borderRadius: 8,
