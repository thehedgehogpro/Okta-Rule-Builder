(function () {
    ...
    // Injects an "Open in Rule Builder" launcher into Okta's native Add/Edit
    // Group Rule modal, immediately before the Save button. The modal is created
    // on demand (and destroyed/recreated each time), so we watch for it with a
    // MutationObserver rather than a one-shot poll.
    function injectGroupRuleModalButton() {
        // The Add/Edit Rule modal has a Save button; the read-only "View Rule"
        // modal has only a Close button. Anchor on whichever exists so both
        // modals get the launchers.
        ...
        const MARK = 'rs-rulebuilder-launcher';

        function tryInject() {
            // Prefer the Save button (Edit/Add). Fall back to Close (View Rule).
            const anchorBtn = document.querySelector(SAVE_SEL) || document.querySelector(CLOSE_SEL);
            if (!anchorBtn) return;
            ...
            // Place it right in front of the anchor button.
            anchorBtn.parentNode.insertBefore(launch, anchorBtn);

            launch.addEventListener('click', function () {
                const expr = document.querySelector(EXPR_SEL);
                if (typeof createGroupRuleBuilder !== 'function') {
                    const errPopup = createPopup('Okta Rule Builder');
                    errPopup.html('Okta Rule Builder failed to load. Ensure group-rule-builder.js is included and loaded before rockstar.js.');
                    return;
                }

                const builderPopup = createPopup('Okta Rule Builder');
                // Okta's Edit/Add Rule modal is an aria-modal dialog at
                // z-index 1002 with a dimming backdrop above rockstar's default
                // popup z-index (1000). Raise this popup above the modal so it
                // isn't greyed out or click-blocked by the overlay.
                builderPopup.parent().css({zIndex: 2000});
                const container = builderPopup[0].appendChild(document.createElement('div'));
                container.style.minWidth = '820px';

                const builder = createGroupRuleBuilder(container);

                // Detect which IF mode the rule form is in.
                const advancedRadio = document.querySelector('input[data-se-name="__activeBuilder__"][value="EDITOR"]');
                const isAdvanced = !!(advancedRadio && advancedRadio.checked);

                if (isAdvanced && expr && expr.value.trim()) {
                    // Advanced ("Use Okta Expression Language") is active: copy the
                    // raw expression into the Import expression box and auto-parse
                    // it so the preview is shown without a manual click.
                    const res = builder.importExpressionText(expr.value);
                    if (res && !res.ok) {
                        const note = document.createElement('div');
                        note.style.cssText = 'color:#b00;font-size:12px;margin:8px 0;';
                        note.textContent = 'Could not parse the existing expression. (' + res.error + ')';
                        container.insertBefore(note, container.firstChild);
                    }
                } else if (expr && expr.value.trim()) {
                    // Basic mode (or no explicit mode): parse straight into the builder tree.
                    const res = builder.loadExpression(expr.value);
                    if (!res.ok) {
                        const note = document.createElement('div');
                        note.style.cssText = 'color:#b00;font-size:12px;margin:8px 0;';
                        note.textContent = 'Could not parse the existing expression; starting from a blank builder. (' + res.error + ')';
                        container.insertBefore(note, container.firstChild);
                    }
                }

                // "Apply to Okta" writes the generated expression back into the
                // textarea and fires an input event so Okta's form model updates.
                ...
                apply.addEventListener('click', function () {
                    ...
                    // Make sure Okta is in "advanced / expression" mode so the
                    // textarea is the field that gets saved.
                    const advanced = document.querySelector('input[data-se-name="__activeBuilder__"][value="EDITOR"]');
                    ...
                    builderPopup.parent().find('.close').click();
                });
                applyBar.appendChild(apply);
                builderPopup.append(applyBar);
            });

            // Second launcher: "Preview OEL Rule" — lists the users that match
            // the current expression. Placed right after the Rule Builder button.
            ...
            preview.addEventListener('click', function () {
                const expr = document.querySelector(EXPR_SEL);
                if (typeof createOelPreview !== 'function') {
                    const errPopup = createPopup('OEL Preview');
                    errPopup.html('OEL Preview failed to load. Ensure oel-preview.js is included and loaded before rockstar.js.');
                    return;
                }
                if (!expr || !expr.value.trim()) {
                    const errPopup = createPopup('OEL Preview');
                    errPopup.html('There is no expression to preview. Switch the IF condition to "Use Okta Expression Language" and enter a rule first.');
                    return;
                }

                const previewPopup = createPopup('OEL Preview');
                // Raise above Okta's aria-modal dialog (z-index 1002), same as
                // the Rule Builder launcher does.
                previewPopup.parent().css({zIndex: 2000});
                const container = previewPopup[0].appendChild(document.createElement('div'));
                container.style.minWidth = '820px';

                // Hand rockstar's own request helpers to the tool so it uses the
                // extension's headers, XSRF token, and Link-header parsing.
                createOelPreview(container, {
        ...
        // Catch a modal that is already open, then watch for future ones.
        tryInject();
    ...
    function exportObjects() {
        ...
        if (location.pathname == "/admin/users") {
            ...
            createDiv("Export Group Rules", mainPopup, function () {
             ...
             createDiv("Okta Rule Builder", mainPopup, function () {
            if (typeof createGroupRuleBuilder !== "function") {
                const errPopup = createPopup("Okta Rule Builder");
                errPopup.html("Okta Rule Builder failed to load. Ensure group-rule-builder.js is included in the extension.");
                return;
            }
            const builderPopup = createPopup("Okta Rule Builder");
            // Give the builder room to render its own UI.
            const container = builderPopup[0].appendChild(document.createElement("div"));
            container.style.minWidth = "820px";
            createGroupRuleBuilder(container);
        });
        } else if (location.pathname == "/admin/apps/active") {
