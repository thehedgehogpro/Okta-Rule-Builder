...
const Check = p => E(Icon, p, E("polyline", {
  points: "20 6 9 17 4 12"
}));
const FolderTree = p => E(Icon, p, E("path", {
  d: "M20 10a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1h-2.5a1 1 0 0 1-.8-.4l-.9-1.2A1 1 0 0 0 14 3h-2a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1Z"
}), E("path", {
  d: "M20 21a1 1 0 0 0 1-1v-3a1 1 0 0 0-1-1h-2.9a1 1 0 0 1-.88-.55l-.42-.85a1 1 0 0 0-.92-.6H12a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1Z"
}), E("path", {
  d: "M3 5a2 2 0 0 0 2 2h3"
}), E("path", {
  d: "M3 3v13a2 2 0 0 0 2 2h3"
}));
const User = p => E(Icon, p, E("path", {
...
const Layers = p => E(Icon, p, E("polygon", {
...
const STRING_OPS = [{
  ...
  value: "sw",
  label: "starts with",
  tpl: (l, v) => `String.startsWith(${l}, "${v}")`
}, {
  value: "ew",
  label: "ends with",
  tpl: (l, v) => `String.endsWith(${l}, "${v}")`
}, {
  value: "contains",
  label: "contains",
  tpl: (l, v) => `String.stringContains(String.toLowerCase(${l}), "${v.toLowerCase()}")`
}, {
  value: "present",
  label: "is present",
  tpl: l => `String.stringContains(${l}, "") || ${l} != null`
}, {
  value: "empty",
  label: "is empty / null",
  tpl: l => `String.isNullOrEmpty(${l})`
}];
const GROUP_OPS = [{
  value: "member",
...
function leafToEL(leaf) {
  ...
  return op.tpl(attr, leaf.value.trim());
}
  ...
  function parseComparisonOrCall(negated) {
    ...
    if (peek() && peek().t === "(") {
      ...
      if (peek() && (peek().t === "==" || peek().t === "!=")) {
        ...
        const valTok = peek();
        let val = "";
        if (valTok && (valTok.t === "str" || valTok.t === "id")) {
          next();
          val = valTok.v;
        }
    ...
    if (peek() && (peek().t === "==" || peek().t === "!=")) {
      ...
      const valTok = peek();
      let val = "";
      if (valTok && valTok.t === "str") {
        next();
        val = valTok.v;
      } else if (valTok && valTok.t === "id") {
        next();
        val = valTok.v;
      }
      const leaf = newLeaf();
...
function applyAttr(leaf, attr) {
  ...
  if (known) {
    ...
    leaf.attr = "__custom__";
    leaf.customAttr = attr;
  }
}
...
function funcToLeaf(name, args, negated) {
  ...
  if (name === "String.startsWith" || name === "String.endsWith") {
    const attrTok = (args[0] || []).find(t => t.t === "id");
    const leaf = newLeaf();
    leaf.type = "profile";
    leaf.op = name === "String.startsWith" ? "sw" : "ew";
...
function normalizeELText(src) {
  const replacements = [[/\bdoes\s+not\s+equal\b/gi, "!="], [/\bnot\s+equals?\b/gi, "!="], [/\bequals\b/gi, "=="], [/\bequal\b/gi, "=="],
...
function parseEL(src) {
  ...
  try {
    ...
    let root;
    if (ast.kind === "group") root = ast;else root = {
      id: uid(),
      kind: "group",
      join: "AND",
      children: [ast]
    ...
    const raws = [];
    const scan = nd => {
      if (nd.kind === "group") nd.children.forEach(scan);else if (nd._raw) raws.push(nd._raw);
    };
    scan(root);
    return {
      root,
...
function Leaf({
  ...
  const op = node.type === "profile" ? STRING_OPS.find(o => o.value === node.op) : GROUP_OPS.find(o => o.value === node.op);
  const hideValue = node.type === "profile" && ["present", "empty"].includes(node.op);
  if (node._raw !== undefined) {
    ...
    }, "Raw EL"), /*#__PURE__*/React.createElement("input", {
      value: node._raw,
      onChange: e => set({
        _raw: e.target.value
      }),
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      color: node.type === "profile" ? "#04231b" : C.dim,
      fontWeight: 600
    }
    ...
    style: {
      ...
      color: node.type === "group" ? "#04231b" : C.dim,
      fontWeight: 600
    }
  }, /*#__PURE__*/React.createElement(Layers, {
    size: 13
  }), " Group")), node.type === "profile" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("select", {
    value: node.attr,
    onChange: e => set({
      attr: e.target.value
    }),
    style: selStyle
  }, PROFILE_ATTRS.map(a => /*#__PURE__*/React.createElement("option", {
    key: a.value,
    value: a.value
  }, a.label))), node.attr === "__custom__" && /*#__PURE__*/React.createElement("input", {
    placeholder: "user.customField",
    value: node.customAttr,
    onChange: e => set({
      customAttr: e.target.value
    }),
    style: {
      ...selStyle,
      width: 150
    ...
    style: selStyle
  }, STRING_OPS.map(o => /*#__PURE__*/React.createElement("option", {
    key: o.value,
    value: o.value
  }, o.label))), !hideValue && /*#__PURE__*/React.createElement("input", {
    placeholder: "value",
    value: node.value,
    onChange: e => set({
    ...
    style: selStyle
  }, GROUP_OPS.map(o => /*#__PURE__*/React.createElement("option", {
    key: o.value,
    value: o.value
...
function Group({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(JoinToggle, {
    join: node.join,
    onChange: j => set({
      join: j
    })
  ...
  }, "match ", node.join === "AND" ? "all" : "any", " of the following"), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1
    }
  }), canRemove && /*#__PURE__*/React.createElement("button", {
  ...
  }, "Remove Logic")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8
    }
  }, node.children.map((child, i) => child.kind === "group" ? /*#__PURE__*/React.createElement(Group, {
    key: child.id,
    node: child,
    depth: depth + 1,
    canRemove: true,
    ...
    size: 14
  }), " Condition"), /*#__PURE__*/React.createElement("button", {
    onClick: () => set({
...
// ---- Builder panel (the original UI, now driven by lifted state) -----------
function BuilderPanel({
  root,
  setRoot
}) {
  const [copied, setCopied] = useState(false);
  const expression = useMemo(() => generateEL(root), [root]);
  const copy = () => {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Okta Expression Language"), /*#__PURE__*/React.createElement("button", {
    onClick: copy,
    disabled: !expression,
    style: {
      display: "flex",
      alignItems: "center",
      gap: 6,
      background: copied ? C.accent : C.panel2,
      color: copied ? "#04231b" : C.text,
      border: `1px solid ${C.border}`,
      borderRadius: 8,
      cursor: expression ? "pointer" : "not-allowed",
      opacity: expression ? 1 : 0.5,
      fontSize: 12,
      padding: "7px 12px",
      fontWeight: 600
    }
    ...
    style: {
      ...
      fontSize: 14,
      lineHeight: 1.6,
      color: expression ? C.accent : C.dim,
      fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace",
      whiteSpace: "pre-wrap",
      wordBreak: "break-word",
      minHeight: 24
    }
  }, expression || "// Complete the conditions to generate an expression"), /*#__PURE__*/React.createElement("p", {
    style: {
      color: C.dim,
      fontSize: 11,
      marginTop: 8,
  ...
  }, "Paste into Directory \u2192 Groups \u2192 Rules \u2192 \u201CCreate Rule\u201D. Verify with the Okta Expression Language preview before activating.")));
}

// ---- Import panel (EL text -> parse -> load into builder) ------------------
function ImportPanel({
  onImport
}) {
  const [text, setText] = useState("");
  const [result, setResult] = useState(null); // {root, warnings} | {error}

  const preview = () => setResult(parseEL(text));
  const load = () => {
    const r = result || parseEL(text);
    if (r && r.root) onImport(r.root);
  };
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Paste an Okta Expression"), /*#__PURE__*/React.createElement("textarea", {
    value: text,
    onChange: e => {
  ...
  }, "Parse & preview"), /*#__PURE__*/React.createElement("button", {
    onClick: load,
    disabled: !ok,
    style: {
      display: "flex",
      alignItems: "center",
      gap: 6,
      background: ok ? C.accent : C.panel2,
      color: ok ? "#04231b" : C.dim,
      border: `1px solid ${C.border}`,
      borderRadius: 8,
    ...
    size: 14
  }), " Load into builder"))), result && result.error && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 16,
      background: "#3a1414",
  ...
  }, result.error), ok && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 16
    }
  }, result.warnings && result.warnings.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      background: "#3a2f10",
      border: "1px solid #7a5a1e",
      borderRadius: 12,
  ...
  }, result.warnings.length, " fragment", result.warnings.length > 1 ? "s" : "", " couldn't be mapped to a UI control and will load as editable \u201CRaw EL\u201D rows. Everything else parsed cleanly."), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      textTransform: "uppercase",
      letterSpacing: 1,
  ...
  }, "Re-generated expression (round-trip check)"), /*#__PURE__*/React.createElement("pre", {
    style: {
      background: "#061426",
      border: `1px solid ${C.border}`,
      borderRadius: 12,
  ...
  }, generateEL(result.root)), /*#__PURE__*/React.createElement("p", {
    style: {
      color: C.dim,
      fontSize: 11,
      marginTop: 8,
...
function OktaRuleBuilder() {
  const [tab, setTab] = useState("build"); // "build" | "import"
  const [root, setRoot] = useState(() => newGroup("AND", [{
    ...newLeaf(),
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Okta Group Rule Builder"), /*#__PURE__*/React.createElement("p", {
    style: {
      color: C.dim,
      fontSize: 13,
      margin: "4px 0 0"
    }
  }, "Compose AND / OR conditions on user profile attributes and group membership, or import an existing expression to edit it.")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 4,
    ...
    id: "import",
    label: "Import expression"
  })), tab === "build" ? /*#__PURE__*/React.createElement(BuilderPanel, {
    root: root,
    setRoot: setRoot
  }) : /*#__PURE__*/React.createElement(ImportPanel, {
    onImport: importRoot
  })));
}
const rootEl = document.getElementById("root");
ReactDOM.createRoot(rootEl).render(React.createElement(OktaRuleBuilder));